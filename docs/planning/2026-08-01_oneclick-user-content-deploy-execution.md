...
| B19 | **로그인 세션이 수 분 만에 끊김** — 인증 쿠키(`sb-*-auth-token.0/.1`)는 남아 있는데 서버가 401을 반환하는 "좀비 세션". 2026-08-01 라이브 테스트 중 2회 재현(각각 로그인 후 5~10분 내). 자동화가 API를 빠르게 연속 호출한 것이 리프레시 토큰 회전 경합을 유발했을 가능성이 있어 일반 사용 조건과는 다를 수 있음 → **원인 미확정, 별도 조사 필요**. 참고: [[auth-self-healing]], 2026-07-12 키 로테이션 이력 | 중간 (조사) | 미정 |
