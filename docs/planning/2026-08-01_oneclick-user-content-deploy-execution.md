...
- 검증: typecheck 클린 · **vitest 512/512** · lint 에러 0 (프레임워크 감지 테스트 21건 신설)
- 잔여: 실제 빌드형 저장소 e2e는 미수행 — 라이브 검증 대상

### Phase 3 — 퍼널 브릿지 (미착수)

- 모체 기획 문서 §7 참조 (서비스 시그니처 스캔 → 서비스맵 자동 제안, showcase 소스 뱃지)
...
- **다음**: 빌드형 저장소 라이브 e2e → Phase 3(퍼널 브릿지).
