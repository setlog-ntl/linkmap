...
- 잔여(보고만): `project_services`의 UNIQUE 제약이 M094에서 제거돼 동시 요청 시 중복 행 가능(영향은 서비스맵 노드 중복). `homepage_deploys` RLS에 `WITH CHECK` 부재는 별도 마이그레이션 사안. 라이브 e2e 미수행
...
- **다음**: Phase 3 라이브 검증 → 원클릭 "내 자료 배포" 전체 트랙 완료.
