...
Linkmap 개발 8개월째, 저는 한 가지 문제에 3개월째 막혀 있었습니다. Cursor로 코드를 짜고, Vercel에 배포하고, 사용자도 조금씩 생겼는데 특정 기능이 개발 환경에서는 되고 배포 환경에서는 조용히 죽어버렸습니다. 에러 메시지도 없었습니다. AI에게 열 번쯤 물어봤지만 원인을 못 찾았습니다.
...
저는 SAP ERP를 10년 가까이 운영했습니다. ERP는 모듈 간 데이터 흐름이 엄격합니다. 구매 발주가 생기면 재고가 예약되고, 입고 확인이 되면 회계 전표가 자동 생성됩니다. 이 관계를 이해하지 못하면 오류가 연쇄적으로 발생합니다.
...
코스피 상장사에서 SAP ERP 시스템을 운영하는 담당자입니다. 코딩이라고는 대학교 1학년 때 C언어 수업이 전부였습니다. 그런 제가 [Claude Code](https://claude.ai/code)와 [Supabase](https://supabase.com)를 써서 Linkmap이라는 웹서비스를 만들었습니다.
...
Linkmap에 구글 로그인을 추가하기로 했습니다. AI에게 물어봤더니 코드는 20분 만에 나왔습니다. 하지만 실제로 동작하기까지는 사흘이 걸렸습니다.
