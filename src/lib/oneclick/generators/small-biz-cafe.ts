...
// small-biz와 동일한 로직을 사용하되 기본값이 카페(온기 로스터리)에 맞춤
// ──────────────────────────────────────────────
...
function generateConfigTs(state: ModuleConfigState): string {
  ...
  const name = (hero.name as string) || '온기 로스터리';
  const nameEn = (hero.nameEn as string) || 'Ongi Roastery';
  const description = (hero.description as string) || '매일 아침, 직접 로스팅한 한 잔의 커피';
  const descriptionEn = (hero.descriptionEn as string) || 'A cup of freshly roasted coffee every morning';
  const phone = (hero.phone as string) || '02-338-1204';
  const primaryColor = (hero.primaryColor as string) || '#8b6914';
  const fontFamily = (hero.fontFamily as string) || 'Pretendard';
  const designPreset = (hero.designPreset as string) || 'default';

  const address = (location.address as string) || '서울 마포구 연남로 23길 8, 1층';
  const addressEn = (location.addressEn as string) || '1F, 8, Yeonnam-ro 23-gil, Mapo-gu, Seoul';
