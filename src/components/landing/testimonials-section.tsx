        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {testimonials.map((item, i) => (
            <ScrollReveal key={item.nameKey} delay={i * 0.1}>
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 backdrop-blur-sm p-6 h-full flex flex-col transition-all hover:border-zinc-700">
                ...
                <div>
                  <p className="text-sm font-bold text-white">{t(locale, item.nameKey)}</p>
                  <p className="text-xs text-gray-400">{t(locale, item.roleKey)}</p>
                </div>
              </div>
            </ScrollReveal>
          ))}
