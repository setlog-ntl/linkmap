...
function ServiceMapVisual() {
  return (
    <div className="h-48 w-full rounded-lg border border-[#dde0e7] bg-[#fafbfc] dark:border-white/10 dark:bg-[#0f172a] p-4 relative overflow-hidden">
      <div className="absolute inset-0 flex items-center justify-center opacity-70">
        <svg viewBox="0 0 400 200" width="100%" height="100%">
          {/* Central node */}
          <rect x="155" y="78" width="90" height="44" rx="8" fill="var(--landing-card-bg)" stroke="hsl(220,60%,35%)" strokeWidth="2" />
          <text x="200" y="104" textAnchor="middle" fill="hsl(220,60%,35%)" fontSize="10" fontFamily="monospace">Next.js</text>

          {/* Source node */}
          <circle cx="60" cy="100" r="18" fill="var(--landing-card-bg)" stroke="hsl(220,60%,35%)" strokeWidth="2" />
...
function EnvVarVisual() {
  return (
    <div className="flex-1 rounded-lg bg-[#fafbfc] border border-[#dde0e7] dark:bg-[#0f172a] dark:border-white/10 p-4 font-mono text-xs text-[#1a2740]/70 dark:text-[#e2e8f0]/70 shadow-inner">
      ...
      <div className="space-y-1">
        ...
        <p>{'}'}</p>
        <p className="text-[#63738a] dark:text-[#94a3b8] mt-2">{'// Variables injected securely at runtime'}</p>
      </div>
    </div>
  );
}
...
function ChecklistVisual({ locale }: { locale: 'ko' | 'en' }) {
  ...
  return (
    <div className="mt-6 flex flex-col gap-3">
      {items.map((item, i) => (
        <div key={i} className={`flex items-center gap-3 rounded-lg bg-[#fafbfc] dark:bg-[#0f172a] p-3 border border-[#dde0e7] dark:border-white/10 transition-colors ${item.done ? 'hover:border-[hsl(220,60%,35%)]/30' : 'opacity-50'}`}>
          <CheckCircle2 className={`w-4 h-4 shrink-0 ${item.done ? 'text-[#2bee79]' : 'text-[#c8cdd6] dark:text-[#475569]'}`} />
          <span className={`text-xs font-mono ${item.done ? 'text-[#1a2740]/70 dark:text-[#e2e8f0]/70' : 'text-[#c8cdd6] dark:text-[#475569]'}`}>{item.text}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="mt-6 grid grid-cols-2 gap-3">
      {templates.map((tmpl, i) => (
        <div key={i} className={`flex flex-col items-center justify-center rounded-lg border border-[#dde0e7] dark:border-white/10 bg-gradient-to-br ${tmpl.color} p-4 transition-colors hover:border-[hsl(220,60%,35%)]/30`}>
          <div className="h-8 w-8 rounded-lg bg-[#dde0e7] dark:bg-[#475569] mb-2" />
          <span className="text-[10px] text-[#63738a] dark:text-[#94a3b8] font-medium">{tmpl.name}</span>
        </div>
      ))}
    </div>
  );
...
const featureColors = [
  { bg: 'bg-[hsl(220,60%,92%)] dark:bg-[hsl(220,60%,20%)]', text: 'text-[hsl(220,60%,35%)] dark:text-[hsl(220,60%,70%)]' },
...
export function FeaturesBento() {
  ...
  return (
    <section className="py-24 bg-white dark:bg-[#111827]" id="features">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <ScrollReveal>
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl font-bold tracking-tight sm:text-4xl text-[#1a2740] dark:text-[#e2e8f0]">{t(locale, 'landing.featuresTitle')}</h2>
            <p className="mt-4 text-lg text-[#63738a] dark:text-[#94a3b8] max-w-2xl mx-auto">{t(locale, 'landing.featuresDesc')}</p>
          </div>
        </ScrollReveal>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <ScrollReveal className="col-span-1 md:col-span-2" delay={0.1}>
            <div className="group relative overflow-hidden rounded-2xl border border-[#dde0e7] bg-white dark:border-white/10 dark:bg-[#0f172a] p-8 transition-all hover:shadow-md hover:-translate-y-0.5 h-full">
              <div className="relative flex flex-col h-full justify-between">
                <div>
                  ...
                  <h3 className="text-xl font-bold text-[#1a2740] dark:text-[#e2e8f0]">{t(locale, 'landing.featureMapTitle')}</h3>
                  <p className="mt-2 text-sm text-[#63738a] dark:text-[#94a3b8] leading-relaxed">{t(locale, 'landing.featureMapDesc')}</p>
                </div>
                <div className="mt-8">
                  <ServiceMapVisual />
                </div>
          ...
          <ScrollReveal delay={0.2}>
            <div className="group relative overflow-hidden rounded-2xl border border-[#dde0e7] bg-white dark:border-white/10 dark:bg-[#0f172a] p-8 transition-all hover:shadow-md hover:-translate-y-0.5 h-full">
              <div className="relative">
                ...
                <h3 className="text-xl font-bold text-[#1a2740] dark:text-[#e2e8f0]">{t(locale, 'landing.featureChecklistTitle')}</h3>
                <p className="mt-2 text-sm text-[#63738a] dark:text-[#94a3b8] leading-relaxed">{t(locale, 'landing.featureChecklistDesc')}</p>
          ...
          <ScrollReveal delay={0.3}>
            <div className="group relative overflow-hidden rounded-2xl border border-[#dde0e7] bg-white dark:border-white/10 dark:bg-[#0f172a] p-8 transition-all hover:shadow-md hover:-translate-y-0.5 h-full">
              <div className="relative">
                ...
                <h3 className="text-xl font-bold text-[#1a2740] dark:text-[#e2e8f0]">{t(locale, 'landing.featureTemplateTitle')}</h3>
                <p className="mt-2 text-sm text-[#63738a] dark:text-[#94a3b8] leading-relaxed">{t(locale, 'landing.featureTemplateDesc')}</p>
          ...
          <ScrollReveal className="col-span-1 md:col-span-2" delay={0.4}>
            <div className="group relative overflow-hidden rounded-2xl border border-[#dde0e7] bg-white dark:border-white/10 dark:bg-[#0f172a] p-8 transition-all hover:shadow-md hover:-translate-y-0.5">
              <div className="relative flex flex-col md:flex-row h-full gap-8">
                <div className="flex-1">
                  ...
                  <h3 className="text-xl font-bold text-[#1a2740] dark:text-[#e2e8f0]">{t(locale, 'landing.featureEnvTitle')}</h3>
                  <p className="mt-2 text-sm text-[#63738a] dark:text-[#94a3b8] leading-relaxed">{t(locale, 'landing.featureEnvDesc')}</p>
          ...
          <ScrollReveal className="col-span-1 md:col-span-3" delay={0.5}>
            <div className="group relative overflow-hidden rounded-2xl border border-[#dde0e7] bg-gradient-to-br from-white to-[hsl(220,60%,97%)] dark:border-white/10 dark:from-[#111827] dark:to-[hsl(220,60%,12%)] p-8 transition-all hover:shadow-md hover:-translate-y-0.5">
              <div className="relative">
                ...
                <h3 className="text-xl font-bold text-[#1a2740] dark:text-[#e2e8f0]">{t(locale, 'landing.aiSectionTitle')}</h3>
                <p className="mt-2 text-sm text-[#63738a] dark:text-[#94a3b8] leading-relaxed max-w-2xl">{t(locale, 'landing.aiSectionDesc')}</p>
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 md:grid-cols-5 gap-4">
                  {[
                    ...
                    <div key={ai.titleKey} className="rounded-xl bg-white/80 border border-[#dde0e7]/50 dark:bg-white/5 dark:border-white/10 p-4 transition-all hover:border-[hsl(220,60%,35%)]/30">
                      <h4 className="text-sm font-bold text-[#1a2740] dark:text-[#e2e8f0] mb-1">{t(locale, ai.titleKey)}</h4>
                      <p className="text-xs text-[#63738a] dark:text-[#94a3b8] leading-relaxed line-clamp-3">{t(locale, ai.descKey)}</p>
                    </div>
                  ))}
                </div>
              </div>
