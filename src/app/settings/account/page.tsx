...
import {
  ...
  Layers, Trash2, Wrench, ArrowRightLeft,
} from 'lucide-react';
...
export default function AccountPage() {
  ...
  return (
    <div className="min-h-full bg-background">
      ...
      {/* ── 4. Custom Services ── */}
      <CustomServicesSection locale={locale} />
...
function CustomServicesSection({ locale }: { locale: 'ko' | 'en' }) {
  const { data: customServices, isLoading } = useMyCustomServices();
  ...
  const handleDelete = async (id: string, name: string) => {
    try {
      ...
      toast.success(`"${name}" 서비스가 삭제되었습니다`);
    } catch {
  ...
  const handleMigrate = async (customId: string, globalId: string, globalName: string) => {
    try {
      ...
      toast.success(
        locale === 'ko'
          ? `"${globalName}" 글로벌 서비스로 전환되었습니다`
          : `Migrated to global service "${globalName}"`
      );
    } catch (err) {
  ...
  return (
    <section className="mb-12">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-lg font-bold text-foreground">
            {locale === 'ko' ? '내 커스텀 서비스' : 'My Custom Services'}
          </h2>
          <p className="text-[13px] text-muted-foreground mt-1">
            {locale === 'ko'
              ? '카탈로그에 없는 서비스를 직접 등록하여 프로젝트에 연결할 수 있습니다'
              : 'Register services not in the catalog and connect them to projects'}
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={Wrench}
          title={locale === 'ko' ? '등록한 커스텀 서비스가 없습니다' : 'No custom services yet'}
          description={locale === 'ko'
            ? '서비스 추가 시 카탈로그에 없는 서비스를 직접 등록할 수 있습니다'
            : 'You can register custom services when adding services to a project'}
        />
      ) : (
        <div className="rounded-xl border border-border bg-card text-card-foreground overflow-hidden divide-y divide-border">
              ...
              <div key={svc.id}>
                  ...
                  <div className="flex items-center justify-between gap-3 px-5 py-2.5 bg-amber-500/10 border-b border-amber-500/20">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="text-[13px] text-amber-700 dark:text-amber-300 truncate">
                        {locale === 'ko'
                          ? `"${matched.name}"과(와) 유사한 글로벌 서비스가 카탈로그에 있습니다`
                          : `A similar global service "${matched.name}" exists in the catalog`}
                      </span>
                    </div>
                    <ConfirmDialog
                      ...
                      description={
                        locale === 'ko'
                          ? `"${svc.name}" 커스텀 서비스를 "${matched.name}" 글로벌 서비스로 전환합니다. 연결된 프로젝트와 환경변수가 자동으로 이관됩니다.`
                          : `Migrate custom service "${svc.name}" to global service "${matched.name}". All linked projects and env vars will be transferred.`
                      }
                      confirmLabel={locale === 'ko' ? '전환' : 'Migrate'}
                      ...
                      trigger={
                        <Button
                          variant="outline"
                          size="sm"
                          className="h-7 text-xs border-amber-500/30 text-amber-700 dark:text-amber-300 hover:bg-amber-500/15 shrink-0"
                          disabled={migrateMutation.isPending}
                        >
                          <ArrowRightLeft className="h-3 w-3 mr-1.5" />
                          {locale === 'ko' ? '전환' : 'Migrate'}
                        </Button>
                      }
                    />
                  </div>
                ...
                <div className="flex items-center justify-between px-5 py-4 hover:bg-muted/40 transition-colors">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className="h-9 w-9 rounded-lg bg-muted/60 flex items-center justify-center text-lg shrink-0">
                      {svc.icon_emoji || '⚙️'}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-[14px] font-medium text-foreground truncate">{svc.name}</p>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[11px] text-muted-foreground">
                          {categoryLabels[svc.category] || svc.category}
                        </span>
                        <span className="text-[11px] text-muted-foreground/50">
                          {new Date(svc.created_at).toLocaleDateString(locale === 'ko' ? 'ko-KR' : 'en-US')}
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <ConfirmDialog
                      ...
                      description={
                        locale === 'ko'
                          ? `"${svc.name}" 서비스를 삭제하시겠습니까? 이 서비스가 연결된 프로젝트에서도 제거됩니다.`
                          : `Delete "${svc.name}"? It will be removed from all connected projects.`
                      }
